function clusterInterpreter(fitxerInput)

javarmpath '../../../JavaSVN/FloorPlanInterpretation/bin/';
javarmpath '../../../JavaSVN/commons-math3-3.2.jar';
javaaddpath '../../../JavaSVN/FloorPlanInterpretation/bin/';
javaaddpath '../../../JavaSVN/commons-math3-3.2.jar';
...
run('../../../../tools/vlfeat-0.9.16/toolbox/vl_setup');

%% Load JAVA statistics
directory = '../../../groundtruth/svg';
...
if ~strcmp(name,'code') && ~strcmp(name,'dades'),
    load(['../../../Input/500 75000 10000 9 2/' name '/results_final_' name]);
    load(['../../../Input/500 75000 10000 9 2/' name '/results_final2_' name]);
    ...
    clearvars -except jFloorPlan jSTATobj hRAPA x fitxerInput imatgeFinal2...
        pixels fnames name outFolder thrs cont jList
    ...
    bestProb = calculateFPProbability(jFloorPlan,jSTATobj,gmmA,gmmW,gmmD,gmmS);
    ...
    while (allValidMergings.size > 0),
        ...
        for j=0:allValidMergings.size-1;
            ...
            [p] = calculateFPProbability(newFP,jSTATobj,gmmA,gmmW,gmmD,gmmS);
...
save([outFolder '/0results'],'llista','idx','jConfList','image','oldfp','oldprob','imageRooms');
